...
def _target_reviewer(goal: str, actor_id: str, upstream_actor_id: str | None) -> str:
    ...
    return (
        ...
        "- Evidence (code): bullet list of file paths + what you saw\n"
        "- Blockers: list (empty if none)\n"
        "- Non-blocking notes / risks: list (optional)\n"
...
_REVIEWER = RoleTemplate(
    ...
    base_rules_md=(
        ...
        "- Each checklist item should be actionable and ideally reference file paths.\n\n"
        "Output:\n"
        "- Output must match REPORT_FORMAT exactly (one JSON object, no extra text).\n"
    ),
    base_context_md=(
