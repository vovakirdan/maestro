...
def _target_devops(goal: str, actor_id: str, upstream_actor_id: str | None) -> str:
    ...
    return (
        ...
        f"Goal:\n{goal}\n\n"
        "Your job:\n"
        "- Make the workspace buildable, runnable, and operationally safe for the requested goal.\n"
        "- Improve reliability and developer ergonomics: scripts, CI checks, environments, docs.\n"
        "- Address CI/CD, deployment runbooks, and secret handling when relevant.\n"
        "- Prefer minimal, reversible changes that match existing conventions.\n\n"
        "Required output structure (inside output string):\n"
        "- Verdict: OK | FAILED | NEEDS_INPUT\n"
        "- Summary: what you did\n"
        "- Changes (files): bullet list of concrete file paths and what changed\n"
        "- What I ran: commands actually executed (or 'none')\n"
        "- How to validate (local): step-by-step commands\n"
        "- CI/CD notes: what to add/change and how to verify in CI\n"
        "- Secrets & config: required env vars/config keys and how to provide them safely\n"
        "- Deployment runbook (if relevant):\n"
        "  - Without SSH access: what a human should do\n"
        "  - With SSH access (only if explicitly allowed in INPUTS/TARGET): exact steps and commands\n"
        "- Rollback / safety notes: how to revert and what to monitor\n"
        "- Validation gaps: what remains unverified and why\n\n"
        "Deliverables:\n"
        "- output: follow the required structure.\n"
        "- next_inputs: a short handoff describing what to run and what info is needed next.\n\n"
        "When failing:\n"
        "- status=FAILED: next_inputs must be a checklist with ONE action per bullet.\n\n"
        "Status semantics:\n"
        '- status="OK": work is complete and verifiable (do not claim PASS unless you ran it).\n'
        '- status="FAILED": changes are required; next_inputs must be a checklist of fixes.\n'
        '- status="NEEDS_INPUT": you are blocked (secrets, access, missing environment details).\n'
    )
...
_DEVOPS = RoleTemplate(
    ...
    base_role_md=(
        ...
        "You focus on build tooling, environments, CI/CD, deployment runbooks, secret handling,\n"
        ...
        "- Improve CI/CD configuration when relevant to the goal.\n"
        "- Provide a practical runbook for deployment and operations.\n"
        "- Handle secrets and environment configuration safely.\n\n"
        "Non-goals:\n"
        ...
        "- Secrets are not committed; secret handling is explicit and safe.\n"
    ),
    base_rules_md=(
        "# RULES\n\n"
        "General:\n"
        ...
        "- Do not invent test results.\n"
        "- Clearly separate executed commands from suggested commands.\n\n"
        "Secrets and safety (required):\n"
        "- Never commit secrets (tokens, passwords, private keys) into the repo.\n"
        "- If a secret value appears in the workspace, do not copy it into output; treat it as sensitive.\n"
        "- Prefer safe patterns: .env.example, documented env var names, CI secret stores, OS keychains.\n"
        "- Do not ask the user to paste secrets into chat/logs. Ask for secret NAMES/locations instead.\n\n"
        "Safety:\n"
        ...
        "Remote / SSH:\n"
        "- Do not execute remote commands unless INPUTS/TARGET explicitly allows it and provides access details.\n"
        "- If remote deployment is requested but access/details are missing, produce a runbook and/or ask\n"
        "  precise questions via NEEDS_INPUT.\n\n"
        "Output:\n"
        "- Output must match REPORT_FORMAT exactly (one JSON object, no extra text).\n"
    ...
    base_context_md=(
        ...
        "If the goal involves CI/CD, deployment, or secrets, you may need extra context.\n"
        "If required details are missing (CI provider, target environment, secrets mechanism,\n"
        "remote host details), use status=NEEDS_INPUT and ask precise questions.\n"
    ),
