...
class GeminiCLIProvider:
    """
    Provider that shells out to a local Gemini CLI.
    ...
    Note: `-p/--prompt` is deprecated upstream but still useful for feeding large
    prompts via stdin (Gemini appends the prompt flag to stdin content).
    """
    ...
    def run(
        ...
        *,
        ...
        deliver({"type": "provider", "provider": "gemini_cli", "event": "start"})
        ...
        try:
            ...
            raise ProviderError(f"Gemini CLI command not found: {cmd[0]!r}") from e
